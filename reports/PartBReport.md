...
Με βάση τα αποτελέσματα της αξιολόγησης του semantic retrieval με FAISS (Φάση 2),
σε συνδυασμό με τα αντίστοιχα αποτελέσματα της Φάσης 1 (BM25 baseline),
προκύπτουν οι ακόλουθες παρατηρήσεις:

- Στη Φάση 2, οι μετρικές **Precision@k** (`P@5`, `P@10`, `P@15`, `P@20`)
  παραμένουν **σταθερές** για όλα τα εξεταζόμενα retrieval depths
  (`k = 20, 30, 50`). Συγκεκριμένα, για το semantic retrieval παρατηρούνται:
  - **P@5 = 0.60**
  - **P@10 = 0.43**
  - **P@15 ≈ 0.387**
  - **P@20 = 0.355**

- Αντίθετα, στη Φάση 1 (BM25 baseline) οι αντίστοιχες μετρικές ήταν σημαντικά
  υψηλότερες:
  - **P@5 = 0.90**
  - **P@10 = 0.74**
  - **P@15 ≈ 0.713**
  ...
  Η σύγκριση αυτή δείχνει ότι το BM25 επιτυγχάνει σαφώς υψηλότερη ακρίβεια
  στις κορυφαίες θέσεις της κατάταξης σε σχέση με το semantic retrieval,
  ακόμη και όταν στη Φάση 2 χρησιμοποιείται **Pseudo Relevance Feedback (PRF)**.

- Ως προς τη **Mean Average Precision (MAP)**, στη Φάση 2 παρατηρείται
  σταθερή αύξηση με το retrieval depth:
  - **MAP@20 = 0.302**
  - **MAP@30 = 0.346**
  - **MAP@50 = 0.371**

- Στη Φάση 1, η **MAP** είναι αισθητά υψηλότερη για όλα τα αντίστοιχα
  retrieval depths:
  - **MAP@20 ≈ 0.713**
  - **MAP@30 ≈ 0.775**
  - **MAP@50 ≈ 0.792**

  Οι τιμές αυτές δείχνουν ότι το BM25 όχι μόνο τοποθετεί πιο σχετικά έγγραφα
  στις πρώτες θέσεις, αλλά διατηρεί και συνολικά καλύτερη κατάταξη
  σε όλο το εύρος των αποτελεσμάτων.

- Η υστέρηση της Φάσης 2 σε σχέση με τη Φάση 1, παρά τη χρήση embeddings
  και PRF, μπορεί να αποδοθεί σε συνδυασμό παραγόντων:
  
  1. Η **πυκνή σημασιολογική αναπαράσταση** των embeddings οδηγεί σε
     εξομάλυνση των αποστάσεων μεταξύ εγγράφων, μειώνοντας τη διακριτική
     ικανότητα στις κορυφαίες θέσεις της κατάταξης.
  
  2. Το **PRF στο embedding space** βασίζεται σε αρχικά αποτελέσματα που
     δεν είναι πάντα απολύτως σχετικά, με αποτέλεσμα το expanded query
     να υφίσταται μετατόπιση (query drift), επηρεάζοντας αρνητικά την
     ακρίβεια των πρώτων αποτελεσμάτων.
  
  3. Η απουσία ρητής στάθμισης έντονα διακριτικών χαρακτηριστικών
     περιεχομένου οδηγεί σε διαφορετικό τρόπο κατάταξης σε σχέση με
     το BM25, το οποίο ενσωματώνει τέτοιου είδους πληροφορία στο scoring.

- Συνολικά, η σύγκριση των δύο φάσεων δείχνει ότι:
  - το **BM25 (Φάση 1)** υπερέχει ξεκάθαρα τόσο στις μετρικές κορυφής
    (**Precision@k**) όσο και στη συνολική ποιότητα κατάταξης (**MAP**),
  - το **semantic retrieval (Φάση 2)** παρουσιάζει διαφορετικό προφίλ,
    με αυξανόμενη MAP σε μεγαλύτερο βάθος ανάκτησης, αλλά χαμηλότερη
    συνολική απόδοση στο παρόν πειραματικό setup.

Σημαντικό είναι ότι τα παραπάνω αποτελέσματα προκύπτουν από πλήρως
συγκρίσιμο πειραματικό περιβάλλον (ίδια queries, ίδια qrels,
ίδιο `trec_eval`), γεγονός που καθιστά τη σύγκριση μεταξύ των δύο
φάσεων αξιόπιστη και άμεσα ερμηνεύσιμη.
