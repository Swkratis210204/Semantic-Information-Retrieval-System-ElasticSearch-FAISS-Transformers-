...
**Στόχος.** Εφαρμογή των καθαριστικών μετασχηματισμών που υλοποιεί η συνάρτηση `preprocess` στον κώδικα. Κάθε `Text` περνάει από αυτήν τη συνάρτηση πριν γραφτεί στο JSONL· οι καθαριστικές ενέργειες μειώνουν τον "θόρυβο" (noise) στα κείμενα και συνεισφέρουν στη σταθερότητα των μετρικών αξιολόγησης.
...
**Στόχος.** Ανάγνωση και προετοιμασία των ερωτημάτων που θα χρησιμοποιηθούν για την αξιολόγηση της ανάκτησης.

**Όπως υλοποιείται στον κώδικα**
- Τα ερωτήματα φορτώνονται από το αρχείο `queries.csv` με τη συνάρτηση `load_file()` της κλάσης `Evaluation`.  
- Κάθε γραμμή περιέχει ένα **αναγνωριστικό ερωτήματος (qid)** και το **κείμενο του ερωτήματος (query text)**.  
- Δεν εφαρμόζεται επιπλέον προεπεξεργασία στα ερωτήματα, καθώς το Elasticsearch standard analyzer χειρίζεται το tokenization και το lowercasing αυτόματα.

<p align="center">
  <img src="images/load.png" alt="load_file" width="500"/>
</p>

**Συνοπτική ροή αναζήτησης**
- Όλα τα ερωτήματα προέρχονται από το `queries.csv` και εισάγονται στο pipeline με την ίδια μορφή που παρέχεται.  
- Κάθε ερώτημα χρησιμοποιεί την ίδια διαδικασία αναζήτησης (`search_query`) στο πεδίο `text`.

### 3.2 Εκτέλεση Αναζητήσεων (`search_query`)

**Στόχος.** Εκτέλεση των ερωτημάτων από το αρχείο `queries.csv` στο ευρετήριο `ir2025` και ανάκτηση των πιο συναφών εγγράφων βάσει του μοντέλου **BM25**.

**Περιγραφή λειτουργίας**
- Η αναζήτηση πραγματοποιείται μέσω της μεθόδου `search_query(query_text, k, index_name)` της κλάσης `Search`.  
- Η συνάρτηση καλεί το Elasticsearch API χρησιμοποιώντας το query τύπου `match`:

<p align="center">
  <img src="images/search-query.png" alt="search_query" width="500"/>
</p>

- Το πεδίο **`text`** είναι αυτό που έχει ευρετηριαστεί και αξιολογείται με **BM25 similarity**.  
Η παράμετρος **`size=k`** καθορίζει πόσα top-k αποτελέσματα θα επιστραφούν.  

Για κάθε ερώτημα, το αποτέλεσμα περιλαμβάνει:  
- **`_source["id"]`** → το μοναδικό αναγνωριστικό του εγγράφου  
- **`_score`** → τη βαθμολογία σχετικότητας BM25 

### 3.3 Αποθήκευση Αποτελεσμάτων σε TREC Format

**Στόχος.** Αποθήκευση των αποτελεσμάτων κάθε ερωτήματος σε μορφή **TREC format**, ώστε να μπορούν να αξιολογηθούν με εργαλεία όπως το `trec_eval`.

**Περιγραφή λειτουργίας**
- Η αποθήκευση πραγματοποιείται μέσα στη συνάρτηση `generate_file_results()` της κλάσης `Evaluation`.  
- Για κάθε ερώτημα:
  1. Καλείται η `search_query()` για να ληφθούν τα top-*k* αποτελέσματα.  
  2. Τα αποτελέσματα ταξινομούνται κατά φθίνουσα βαθμολογία BM25.  
  3. Κάθε εγγραφή γράφεται σε αρχείο κειμένου με τη μορφή:

**Περισσότερες πληροφορίες για το `trec_eval`**
- Το TREC format αποτελεί τη στάνταρ μορφή εξόδου για αξιολόγηση ανάκτησης.  
- Κάθε γραμμή αντιστοιχεί σε ένα έγγραφο που ανακτήθηκε για ένα ερώτημα, μαζί με τη βαθμολογία σχετικότητας.  
- Αυτή η μορφή χρησιμοποιείται από το `trec_eval` στην επόμενη φάση για να υπολογιστούν οι μετρικές (Precision@k, MAP).

### 3.4 Δημιουργία Αρχείων `results_20`, `results_30`, `results_50`

**Στόχος.** Δημιουργία τριών ξεχωριστών αρχείων αποτελεσμάτων, καθένα από τα οποία περιέχει τα top-*k* έγγραφα για κάθε ερώτημα.

**Περιγραφή λειτουργίας**
- Η μέθοδος `generate_file_results()` δημιουργεί αυτόματα τα εξής αρχεία:
  - `results_20.txt`
  - `results_30.txt`
  - `results_50.txt`
- Για κάθε τιμή *k*, η συνάρτηση:
  1. Εκτελεί όλα τα ερωτήματα.  
  2. Αποθηκεύει τα αποτελέσματα σε TREC format.  
  3. Εκτυπώνει μήνυμα επιτυχίας στο console.

<p align="center">
  <img src="images/203050res.png" alt="result files" width="500"/>
</p>

**Αποτέλεσμα της παραπάνω λειτουργίας**
- Δημιουργήθηκαν τρία αρχεία αποτελεσμάτων που χρησιμοποιούνται στην επόμενη φάση της αξιολόγησης.  
- Η ύπαρξη διαφορετικών τιμών *k* επιτρέπει τη μέτρηση της απόδοσης του συστήματος για διαφορετικά βάθη ανάκτησης.  
- Όλα τα αρχεία περιέχουν **αναγνωριστικά εγγράφων (id)** και **βαθμολογίες BM25** για κάθε ερώτημα.
...
- Σύντομος ορισμός: το `trec_eval` είναι ένα standard εργαλείο για την αξιολόγηση συστημάτων ανάκτησης πληροφορίας που διαβάζει TREC-formatted result files και qrels και επιστρέφει πλήθος μετρικών (MAP, P@k, R@k κ.α.).
- Σημείωση: στο pipeline υπάρχουν δύο τρόποι αξιολόγησης — (α) οι εσωτερικές συναρτήσεις Python (`precision_at_k`, `mean_average_precision`) και (β) το εξωτερικό `trec_eval`.

### 4.2 Εκτέλεση Αξιολόγησης
Η παραγωγή των αποτελεσμάτων ολοκληρώθηκε επιτυχώς: τα TREC-formatted αρχεία `IR2025/results_20.txt`, `IR2025/results_30.txt` και `IR2025/results_50.txt` δημιουργήθηκαν από τη μέθοδο `generate_file_results()`. Ένα απόσπασμα των `results_20.txt` φαίνεται παρακάτω (μορφή `qid Q0 docid rank score method`), επιβεβαιώνοντας τη σωστή μορφοποίηση για αξιολόγηση με `trec_eval`.



Επιπλέον, οι μετρικές P@5, P@10, P@15, P@20 και MAP υπολογίστηκαν εσωτερικά από την κλάση `Evaluation` και ο συνοπτικός πίνακας εμφανίζεται στο notebook (παρακάτω). Τα αποτελέσματα αυτά προκύπτουν απευθείας από τα παραχθέντα `results_k.txt` και το αρχείο `qrels.csv`.
